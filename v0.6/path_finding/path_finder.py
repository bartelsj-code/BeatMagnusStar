from typing import *
from path_finding.player_node import PlayerNode
import heapq
import time, webbrowser
import os
from math import floor
from path_finding.data_access import DataAccess
from path_finding.control_variables import CUTOFF, JUMP_ABOVE, JUMP_BELOW

class PathFinder:
    user_username: str
    dest_username: str
    start_date: tuple[str, str]
    def __init__(self, user_username, dest_username, start_date, game_filter, database_filepath) -> None:
        self.db = DataAccess(database_filepath)
    ...
    def get_valid_neighbors(self, source_node: PlayerNode):
        ...
        for game in relevant_games:
            ...
            if opponent_node in self.frontier_set:
    ...
    def generate_frontier_set(self, node, explore_limit):
        '''
        ...
        while not skipping and depth <= 3:
            print(f"Producing depth {depth} frontier set:")
            set_being_built = set([])
            ...
            for i, element in enumerate(completed_set):
                ...
                if self.db.request_counter >= explore_limit:
                    print(f"\tSearch space too large, using depth {depth-1} set.                                              ") #this could be more elegant, ik
                    skipping = True
                    break
            if not skipping:
        ...
        self.frontier_set = completed_set
    ...
    def find_path(self, exploration_limit):
        ...
        '''
        Finds Path from User to Dest by first finding all nodes within a certain distance from the destination (probably 2 or 3)
        and using these in a set as a target for A* starting from the user. Heuristic targets 
        ...
        res = self.generate_frontier_set(self.dest_node, exploration_limit)
        if res is not None:
            return res
